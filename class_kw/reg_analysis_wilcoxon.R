...
reg <- read.csv("/Users/gabrielketron/class_kw/reg_kw_test_mnar_fixed.csv")
#rint(colnames(reg))

#reg <- reg[reg$rmse<=20000,]
#,"NoImpute"
#newpredrankings <- na.omit(data.frame(id = subset(reg,reg$Model %in% c("Mixed"))$ID,complexf1 = subset(reg,reg$Model %in% c("Mixed"))$rmse, simplef1 = subset(reg,reg$Model %in% c("Simple"))$rmse, mixedf1 = subset(reg,reg$Model %in% c("NonSimple"))$rmse))
#rint(predrankings)
...
#reg[reg=="NoImpute"] <- "No Imputation"
...
myplot <- ggwithinstats(
    ...
    title = "Friedman Test Pairwise Comparison of Regression RMSE Between Treatments"
)
...
ggsave("/Users/gabrielketron/class_kw/ftreg_rmse_mnar.png")
...
ggsave("/Users/gabrielketron/class_kw/ftreg_training_mnar.png")
...
#r2rankings <- na.omit(data.frame(id = subset(reg,reg$Model %in% c("Mixed"))$ID,
#    complexf1 = subset(reg,reg$Model %in% c("Mixed"))$r2, 
#    simplef1 = subset(reg,reg$Model %in% c("Simple"))$r2, 
#    mixedf1 = subset(reg,reg$Model %in% c("NonSimple"))$r2))
...
#explainedrankings <- na.omit(data.frame(id = subset(reg,reg$Model %in% c("Mixed"))$ID,
#    complexf1 = subset(reg,reg$Model %in% c("Mixed"))$explained_var, 
#    simplef1 = subset(reg,reg$Model %in% c("Simple"))$explained_var, 
#    mixedf1 = subset(reg,reg$Model %in% c("NonSimple"))$explained_var))
...
#sink("/Users/gabrielketron/class_kw/ftreg_explained_results.txt")
