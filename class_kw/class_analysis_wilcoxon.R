...
ori <- read.csv("/Users/gabrielketron/class_kw/class_wilcox_test_mnar.csv")
impute <- read.csv("/Users/gabrielketron/class_kw/class_wilcox_test_mnar.csv")
#rint(colnames(reg))
...
newori <- data.frame(f1=ori$ori)
#print(newori)
newori['Model'] <- 'Original'
#print(ori)
newimpute <- data.frame(f1=impute$impute)
#colnames(ori)<-'rmse'
newimpute['Model'] <- 'Impute'

class <- rbind(newori, newimpute)

#print(reg)
#class[class=="NoImpute"] <- "No Imputation in TPOT"
...
myplot <- ggwithinstats(
    ...
    title = "Wilcoxon Pairwise Comparison of F1-Macro Score Between Treatments"
)

#f1rankings <- na.omit(data.frame(id = subset(class,class$Model %in% c("Mixed"))$ID, complexf1 = subset(class,class$Model %in% c("Mixed"))$f1, simplef1 = subset(class,class$Model %in% c("Simple"))$f1, mixedf1 = subset(class,class$Model %in% c("NonSimple"))$f1))

#print(f1rankings)


ggsave("/Users/gabrielketron/class_kw/wilcoxonclass_f1_mnar.png")
...
#ggsave("/Users/gabrielketron/class_kw/ftclass_duration_mnar.png")
#print(class)
...
#ggsave("/Users/gabrielketron/class_kw/ftclass_RMSEAcc.png")
