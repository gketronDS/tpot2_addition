...
for taskid in ['home', 'hosp', 'rehab', 'los', 'SNF']:
    ...
    csvout = pd.DataFrame(columns=['DatasetID','Exp_Name', 'Triplicate', 'Exp1ImputeRMSEAcc','Exp2train_auroc','Exp2train_accuracy', 
                            ...
                            index=['/'+taskid+'_class_non_simple_1/',
                                    '/'+taskid+'_class_non_simple_2/',
                                    '/'+taskid+'_class_non_simple_3/',
                                     ])
    #print(csvout)
    locallist=[]
        ...
        for iter in ['1/', '2/', '3/']:
            ...
            try:
                ...
                csvout.loc['/'+taskid+'_'+exp+iter] = pd.Series({'DatasetID':taskid,'Exp_Name': exp,'Triplicate': iter,'Exp1ImputeRMSEAcc': est['impute_rmse'],'Exp2train_auroc': est['train_score']['train_auroc'],'Exp2train_accuracy': est['train_score']['train_accuracy'], 
                                    ...
                                    'Exp3ImputeModel': Exp3ImputeModel, 'Exp3ClassifierModel': str(tpot_space['fit_model'][1]).split('(')[0] ,'Exp3duration': tpot_space['duration'], 'Exp3inference_duration': tpot_space['inference_time']})
