...
def loop_through_tasks(experiments, task_id_lists, base_save_folder, num_runs, r_or_c, n_jobs):
    ...
    for taskid in task_id_lists:
        ...
        try:
            ...
            print('score start')
            train_score = score(est, X_train_M, y_train, r_or_c=r_or_c)
            print('train score:', train_score)
            ori_test_score = score(est, X_test, y_test, r_or_c=r_or_c)
            print('original test score:', ori_test_score)
            start2 = time.time()
            imputed_test_score = score(est, X_test_M, y_test, r_or_c=r_or_c)
            stop2 = time.time()
            ...
            if os.path.exists(f"{save_folder}/tpot_space_scores.pkl"):
                os.remove(f"{save_folder}/tpot_space_scores.pkl")
            
            if os.path.exists(f"{save_folder}/tpot_space_fitted_pipeline.pkl"):
                os.remove(f"{save_folder}/tpot_space_fitted_pipeline.pkl")
