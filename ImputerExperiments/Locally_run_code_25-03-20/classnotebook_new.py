...
class_data_new = pd.concat([class_data, class_data_non], axis=0)
...
def display_wilcoxon_results(df, savepath, dataset_list=None):
    ...
    nontemp = nontemp[nontemp.ID.isin(simpletemp.ID.unique().tolist())]
    simpletemp = simpletemp[simpletemp.ID.isin(nontemp.ID.unique().tolist())]
    ...
    for score_type in ['f1', 'auroc', 'accuracy', 'balanced_accuracy', 'logloss', 'training_duration', 'RMSEAcc']:
        ...
        for i, space in enumerate([imputer, complexer, simpler, s_first, non_simpler, no_imp]):
            ...
            if i == 4 or i == 5:
                all_list = nontemp.sort_values(by=['DatasetID','Condition', 'Level', 'Triplicate'], ascending=True)[space].values
                #print(simpletemp.sort_values(by=['DatasetID','Condition', 'Level', 'Triplicate'], ascending=True)[space].values)
            else:
    ...
    full_frame.to_csv('/common/ketrong/tpotexp/tpot2/ImputerExperiments/data/c/class_kw_test_mnar.csv')
