...
def loop_through_tasks(experiments, base_save_folder, num_runs, r_or_c, n_jobs):
    ...
    try:
        ...
        if exp['exp_name'] == 'class_simple' or exp['exp_name'] == 'reg_simple':
            ...
            print(auto_space)
            '''
            first_scores['impute_rmse'] = 0
            first_scores['impute_space'] = 0
        ...
        print('score start')
        train_score = score(est, X_train_u, y_train, r_or_c=r_or_c)
        print('train score:', train_score)
        ori_test_score = score(est, X_test_i, y_test, r_or_c=r_or_c)
        print('original test score:', ori_test_score)
        start2 = time.time()
        imputed_test_score = score(est, X_test_u, y_test, r_or_c=r_or_c)
        stop2 = time.time()
