...
reg_data_new = pd.concat([reg_data, reg_data_non], axis=0)
...
for twos in ['complex', 'simple', 'non_simple']:
    for i in range(1,5):
        ...
        out = mnar_table.to_csv('/common/ketrong/tpotexp/tpot2/ImputerExperiments/data/r/Saved_Analysis/'+name+pipe+title+subtitle+str(i)+'mnaronly.csv')
...
for test in ['Train', 'Test']:
    for scoring in ['rmse', 'explained_var', 'r2', 'training_duration', 'RMSEAcc']:
        display_scores_over_options(reg_data_new, test=test, score_type=scoring, savepath='/common/ketrong/tpotexp/tpot2/ImputerExperiments/data/r/Saved_Analysis/')
'''
def display_wilcoxon_results(df, savepath, dataset_list=None):
    ...
    full_frame.to_csv('/common/ketrong/tpotexp/tpot2/ImputerExperiments/data/r/reg_kw_test_mnar.csv')
